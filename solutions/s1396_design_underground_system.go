/*
	...
	An underground railway system is keeping track of customer travel times between
		different stations.
	They are using this data to calculate the average time it takes to travel from
	...
	Implement the UndergroundSystem class:
		...
		double getAverageTime(string startStation, string endStation)
			...
			that happened directly, meaning a check in at startStation followed by a
				check out from endStation.
			...
			There will be at least one customer that has traveled from startStation to
				endStation before getAverageTime
			is called.
