/*
	...
	There are n friends that are playing a game. The friends are sitting in a
		circle and are numbered
	from 1 to n in clockwise order. More formally, moving clockwise from the ith
		friend brings you to
	the (i+1)th friend for 1 <= i < n, and moving clockwise from the nth friend
...
func roll(ps []int, from, k int) []int {
	if len(ps) == 1 {
		return ps
	}
	toDel := (from + k - 1) % len(ps)
	RemoveFromIntSlice(&ps, toDel)
	return roll(ps, toDel%len(ps), k)
}
